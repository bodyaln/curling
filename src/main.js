...
async function init(selectedConfig, initialTurnPlayer) {
    ...
    config =
        selectedConfig ||
        (await fetch("/src/config.json").then(
            (
                response, // Загружаем конфиг игры из JSON
            ) => response.json(),
        )); // Загружаем конфиг
    ...
    currentPlayerIndex = (initialTurnPlayer || config.firstPlayer) === "B" ? 1 : 0; // Выбираем игрока, который ходит первым
    targetVisual = {
    ...
    function createBoundaries() {
        ...
        return [
            ...
            Bodies.rectangle(fieldWidth + 20, fieldHeight / 2, 40, fieldHeight, {
    ...
    function updateStatus() {
        ...
        if (!currentStone) {
            ...
            turnIndicator.textContent = "";
            statusMessage.textContent = "Hra sa skončila. Všetky kamene boli hodené.";
    ...
    function applyServerThrow(payload) {
        ...
        Body.setPosition(
            stone,
            payload.startPosition || getStartPosition(payload.player),
        );
    ...
    function updateStatusPlayerNames() {
        if (statusPlayerAName) statusPlayerAName.textContent = getPlayerName("A");
        if (statusPlayerBName) statusPlayerBName.textContent = getPlayerName("B");
    }
